...
function SectionHeader({ number, title, subtext }: { number: number; title: string; subtext?: string }) {
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-3">
        <span className="flex size-7.5 shrink-0 items-center justify-center rounded-full bg-wale-700 text-xs font-bold text-wale-gold">
          {number}
        </span>
        <p className="font-display text-lg font-bold tracking-[-0.036px] text-wale-800">{title}</p>
      </div>
      {subtext && <p className="text-sm leading-relaxed text-wale-800/60">{subtext}</p>}
    </div>
  );
}
...
export default function PartnerEnquiryForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!HUBSPOT_PORTAL_ID || !HUBSPOT_FORM_GUID) {
      console.error("HubSpot is not configured: missing PUBLIC_HUBSPOT_PORTAL_ID / PUBLIC_HUBSPOT_PARTNER_FORM_GUID");
      setStatus("error");
      return;
    }
  ...
  if (status === "submitted") {
    return (
      <div className="rounded-xl border border-wale-700/15 bg-white px-8 py-12 text-center">
        <p className="font-display text-xl font-bold text-wale-800">Thanks for reaching out!</p>
        <p className="mt-2 text-sm text-wale-800/65">
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-10">
        ...
        <div className="flex flex-col gap-4 sm:flex-row">
          <div className="flex flex-1 flex-col gap-2">
            <label htmlFor="fullName" className={labelClass}>
              Full name <span className="font-bold text-wale-800">*</span>
            </label>
            <input
              id="fullName"
              name="fullName"
          ...
          <div className="flex flex-1 flex-col gap-2">
            <label htmlFor="workEmail" className={labelClass}>
              Work email <span className="font-bold text-wale-800">*</span>
            </label>
            <input
              id="workEmail"
              name="workEmail"
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-1">
            <select
              ...
              className="rounded-lg bg-white px-3 py-5 text-sm text-wale-800 focus:outline-none focus:ring-2 focus:ring-wale-700/30"
            >
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="companyName" className={labelClass}>
            Company name <span className="font-bold text-wale-800">*</span>
          </label>
          <input
            id="companyName"
            name="companyName"
        ...
        <div className="flex flex-col gap-4 sm:flex-row">
          <div className="flex flex-1 flex-col gap-2">
            <label htmlFor="teamSize" className={labelClass}>
              Team size <span className="font-bold text-wale-800">*</span>
            </label>
            <input
              id="teamSize"
              name="teamSize"
          ...
          <div className="flex flex-1 flex-col gap-2">
            <label htmlFor="country" className={labelClass}>
              Country <span className="font-bold text-wale-800">*</span>
            </label>
            <input id="country" name="country" type="text" required placeholder="e.g. Poland" className={inputClass} />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {INTENTS.map(intent => (
            <label
              key={intent.id}
              className="flex cursor-pointer flex-col gap-2 rounded-xl bg-white p-5 has-checked:bg-wale-700"
            >
              <input type="checkbox" name="intents" value={intent.id} className="peer sr-only" />
              <span className="flex size-10 items-center justify-center rounded-full bg-wale-50 peer-checked:bg-wale-peach">
                <img src={intent.icon} alt="" className="size-7 rounded-full" />
              </span>
              <span className="font-display text-sm font-bold text-wale-800 peer-checked:text-white">
                {intent.title}
              </span>
              <span className="text-xs leading-relaxed text-wale-800/60 peer-checked:text-wale-peach">
                {intent.description}
              </span>
            </label>
          ))}
      ...
      <div className="flex flex-col gap-4">
        ...
        <textarea
          ...
          className="w-full rounded-lg bg-white p-4 text-sm text-wale-800 placeholder:text-wale-800/40 focus:outline-none focus:ring-2 focus:ring-wale-700/30"
        />
      </div>
      ...
      <div className="flex flex-col gap-5">
        <label className="flex cursor-pointer items-center gap-3">
          ...
          <span className="flex size-5 shrink-0 items-center justify-center rounded border border-wale-700/30 bg-white text-transparent peer-checked:border-wale-700 peer-checked:bg-wale-700 peer-checked:text-white">
            ✓
          </span>
          <span className="text-sm text-wale-800/70">
            I've read and agree to the <span className="font-bold text-wale-800">Privacy Policy</span>.
          </span>
        </label>
        ...
        <button
          ...
          className="inline-flex w-fit items-center gap-3 rounded-bl-xl rounded-tl-xl rounded-tr-xl bg-wale-700 px-12 py-5 font-display text-base font-medium tracking-[0.08px] text-white hover:bg-wale-800 disabled:opacity-50"
        >
          {status === "sending" ? "Sending…" : "Partner with us"}
          <span aria-hidden="true">→</span>
        </button>
        <p className="text-sm text-wale-800/55">We'll respond within 48 hours. No sales pressure promise.</p>
      </div>
    </form>
  );
}
